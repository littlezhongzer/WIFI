...
import xyz.totok.util.CloseUtils;
...
public final class FileIOUtils {
     ...
     * @return {@code true}: 写入成功<br>{@code false}: 写入失败
     */
    public  boolean writeFileFromString(final String filePath, final String content) {
     ...
     * @return {@code true}: 写入成功<br>{@code false}: 写入失败
     */
    public  boolean writeFileFromString(final String filePath, final String content, final boolean append) {
     ...
     * 设置缓冲区尺寸
     *
     * @param filePath 创建文件夹和文件
     * @param fileName 创建文件夹和文件
     */

    public File makeFilePath(String filePath,String mkName,String fileName) {
        File file0 = null;
        file0 = new File(filePath+mkName);
        if (!file0.exists()) {
            file0.mkdir();
        }
        File file1=null;
        try {
            file1 = new File(filePath+mkName,fileName);
            if (!file1.exists()) {
                file1.createNewFile();
            }
        } catch (Exception e) {
